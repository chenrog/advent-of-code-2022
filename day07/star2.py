...
class Directory(object):
    ...
    def __str__(self):
        return "Directory: " + self.path + " has " + str(
            int(len(self.nestedDirPaths))) + " nested directories and " + str(
            int(len(self.files))) + " files, size: " + str(self.size())
...
currentUnused = totalSpace - directoriesByPath["/"].size()
needToFree = freeSpaceRequired - currentUnused

currentAnswer = totalSpace
for directory in directoriesByPath.values():
    if directory.size() > needToFree and directory.size() < currentAnswer:
